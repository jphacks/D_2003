...
def cosineTheorem(Lidar, radian1, radian2):
    ...
    distance = Lidar[radian1] ** 2 + Lidar[radian2] ** 2 - 2 * Lidar[radian1] * Lidar[radian2] * math.cos(abs(radian2 - radian1))
            ...
            for count, (radian, length) in enumerate(Lidar):
                ...
                for direction in range(4):
                    if len(radian_lists[direction]) > 1:
                        n = combinations_count(len(radian_lists[direction])
                        for num_dis in n, 2):
                            distance = cosineTheorem(Lidar, )
