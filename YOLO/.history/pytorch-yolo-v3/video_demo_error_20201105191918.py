...
def prep_image(img, inp_dim):
    ...
    # global img5
    # img5 = cv2.resize(img5, (inp_dim, inp_dim))
...
if __name__ == '__main__':
    ...
    cap[3] = cv2.VideoCapture(2) #カメラを指定（USB接続）
    ...
    img5 = cv2.imread("images/break.jpg")

    img5 = cv2.resize(img5, (inp_dim[0], inp_dim[0]))
    ...
    while (cap[i].isOpened() for i in range(num_camera)): #カメラが起動している間
        ...
        orig_im = [img5 for i in range(num_camera)]
        ...
        for i in range(num_camera):
            if cap[i].isOpened():
                ...
                if (ret[i]):
                        # 解析準備としてキャプチャ画像を加工
                    # if not (frame[i] == NULL and inp_dim[i] == NULL):
                    img[i], orig_im[i], dim[i] = prep_image(frame[i], inp_dim[i])
                    if CUDA:
                        for i in range(num_camera):
            ...
            if not img[2] == []:
                output1 = model[2](Variable(img[2]), CUDA)
            if not img[3] == []:
                output1 = model[3](Variable(img[3]), CUDA)
