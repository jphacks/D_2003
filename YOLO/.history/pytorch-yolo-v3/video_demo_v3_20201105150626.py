...
if __name__ == '__main__':
    ...
    # print("outpu_type：", type(output))
    # print("outpu_shape：", output.shape)
    ...
    cap[0] = cv2.VideoCapture(1) #カメラを指定（native)
    cap[1] = cv2.VideoCapture(1) #カメラを指定（ivcam）
    ...
    while (cap[i].isOpened() for i in range(num_camera)): #カメラが起動している間
        ...
        if (ret[i] for i in range(num_camera)):
            ...
            """
            if output == []:
                output.append(model[0](Variable(img[0]), CUDA))
                output[0] = write_results(output[0], confidence, num_classes, nms = True, nms_conf = nms_thesh)
            """
            ...
            output0[:,1:5] = torch.clamp(output[:,1:5], 0.0, float(inp_dim[0]))/inp_dim[0]
            output0[:,[1,3]] *= frame[0].shape[1]
            output0[:,[2,4]] *= frame[0].shape[0]
            output1[:,1:5] = torch.clamp(output1[:,1:5], 0.0, float(inp_dim[0]))/inp_dim[0]
            output1[:,[1,3]] *= frame[0].shape[1]
            output1[:,[2,4]] *= frame[0].shape[0]
            ...
            list(map(lambda x: write(x, orig_im[0]), output))
            # list(map(lambda x1: write(x1, orig_im[1]), output1))
            # list(map(lambda x2: write(x2, orig_im[2]), output2))
